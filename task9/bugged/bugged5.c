    ...
    #pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid, i)
    {
        ...
        #pragma omp sections nowait
        {
            #pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    a[i] += DELTA * i;
                omp_unset_lock(&locka); // <- have unlock a-part right here, as we have
                omp_set_lock(&lockb);   // deadlock here
            ...
            #pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    b[i] += PI * i;
                omp_unset_lock(&lockb); // <- have unlock b-part right here, as we have
                omp_set_lock(&locka);   // deadlock here
